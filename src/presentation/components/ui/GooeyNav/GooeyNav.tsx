...
export function GooeyNav({
    ...
    for (let i = 0; i < particleCount; i++) {
      ...
      setTimeout(() => {
        ...
        requestAnimationFrame(() => {
          element.classList.add('gn-active')
        })
        setTimeout(() => {
          try {
            element.removeChild(particle)
          } catch { /* particle already removed */ }
        }, t)
      }, 30)
    }
  }
  ...
  const handleClick = (e: React.MouseEvent<HTMLAnchorElement>, index: number) => {
    ...
    if (filterRef.current) {
      const particles = filterRef.current.querySelectorAll('.gn-particle')
      particles.forEach(p => filterRef.current!.removeChild(p))
    }
    if (textRef.current) {
      textRef.current.classList.remove('gn-text-active')
      void textRef.current.offsetWidth
      textRef.current.classList.add('gn-text-active')
    }
    if (filterRef.current) {
  ...
  return (
    <>
      <style>{`
        :root {
          --gn-linear-ease: linear(0, 0.068, 0.19 2.7%, 0.804 8.1%, 1.037, 1.199 13.2%, 1.245, 1.27 15.8%, 1.274, 1.272 17.4%, 1.249 19.1%, 0.996 28%, 0.949, 0.928 33.3%, 0.926, 0.933 36.8%, 1.001 45.6%, 1.013, 1.019 50.8%, 1.018 54.4%, 1 63.1%, 0.995 68%, 1.001 85%, 1);
        }
        .gn-effect {
          position: absolute;
          opacity: 1;
        ...
        .gn-effect.gn-text.gn-text-active {
          color: black;
        }
        .gn-effect.gn-filter {
        ...
        .gn-item.gn-item-active {
          color: black;
          text-shadow: none;
        }
        .gn-item.gn-item-active::after {
          opacity: 1;
          transform: scale(1);
        }
        .gn-item::after {
        ...
        <nav style={{ transform: 'translate3d(0,0,0.01px)' }}>
          ...
          >
            {items.map((item, index) => (
              <li
                key={item.href}
                className={`gn-item rounded-full relative cursor-pointer text-white/75 ${
                  activeIndex === index ? 'gn-item-active' : ''
                }`}
              >
                <a
                  href={item.href}
                  onClick={e => handleClick(e, index)}
                  className="outline-none py-[0.5em] px-[1.1em] inline-block text-sm font-medium"
                >
                  {item.label}
                </a>
              </li>
